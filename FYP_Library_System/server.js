...
const UserBorrow = require('./models/userBorrowSchema'); // Import UserBorrow model
...
app.post('/borrow', authenticateToken, async (req, res) => {
    const { googleId, returnDate } = req.body; // Expecting googleId and returnDate in the request body

    const userBorrow = new UserBorrow({
        userId: req.user.id, // Get the user's ID from the token
        googleId,
        returnDate: new Date(returnDate) // Store return date
    });

    try {
        await userBorrow.save();
        res.status(201).json(userBorrow); // Return the saved borrow entry
    } catch (error) {
...
// Route to get borrowed books for the authenticated user
app.get('/userBorrows', authenticateToken, async (req, res) => {
    try {
        const borrows = await UserBorrow.find({ userId: req.user.id })
            .populate('googleId'); // Fetch book details if you reference the Book model
