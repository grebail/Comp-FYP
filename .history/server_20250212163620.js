...
const setMidnight = (date) => {
    ...
    return midnightDate;
        ...
        res.status(200).json({
            ...
            borrowedCopies,
            redirectUrl: `user_borrow_copy.html?userid=${userid}&isbn=${isbn}&copies=${borrowedCopies
...
app.put('/api/userBorrows/:borrowRecordId/copies/:copyId/return', authenticateToken, async (req, res) => {
    const { borrowRecordId, // API endpoint to borrow a book by copyId
app.post('/api/books/copy_borrow', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const copiesToBorrow = copyId ? [copyId] : selectedCopies;
        ...
        book.copies.forEach(copy => {
            if (copiesToBorrow.includes(copy.copyId)) {
                if (copy.availability) {
                    ...
                    copy.borrowedDate = setMidnight(new Date());
                    copy.dueDate = setMidnight(new Date(Date.now() + 14 * 24 * 60 * 60 * 1000)); // 14 days from today
                    copy.returned = false;
